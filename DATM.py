...
def main(args):
    ...
    if args.dataset in ["CIFAR10", "CIFAR100","MNIST"] and not args.zca:
        expert_dir += "_NO_ZCA"
    ...
    for it in range(0, args.Iteration+1):
        ...
        for step in range(args.syn_steps):
            ...
            ce_loss = criterion(x, this_y).astype(torch.float32)

            grad = torch.autograd.grad(ce_loss, student_params[-1].astype(torch.float32), create_graph=True)[0]
